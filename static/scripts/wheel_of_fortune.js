...
(function (jq) {
    ...
    jq.wheel_of_fortune = function (element_id, data, callback)
    {
        ...
        var numWedges = Array.from(data).length;
        ...
        function format_item(str_item_name) {

            const splitted_name = str_item_name
                .toUpperCase()
                .split("");
            return splitted_name.join("\n");

        }
        function addWedge(item, index) {
            var s = getRandomColor();
            var reward = format_item(String(item));
            var r = s[0];
            var g = s[1];
            var b = s[2];
            ...
            var wedge = new Konva.Group({
                rotation: (2 * index * Math.PI) / numWedges,
            });
        ...
        function animate(frame) {
            ...
            var angularVelocityChange =
                (angularVelocity * frame.timeDiff * (1 - angularFriction)) / 10000;
